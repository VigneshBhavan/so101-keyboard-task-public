...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--task-config", type=Path, help="Nominal keyboard pose and reset randomization JSON")
    parser.add_argument("--physics-config", type=Path, help="Validated public Newton physics JSON")
    parser.add_argument(
        "--external_callback",
        default=None,
...
def run(argv: list[str]) -> None:
    ...
    if args_cli.task_config:
        task_module = import_local_module("so101_public_task", RL_ROOT / "rsl_rl" / "public_task.py")
        task_data = task_module.load_config(args_cli.task_config)
        task_module.apply_config(env_cfg, task_data)
    physics_data = None
    if args_cli.physics_config:
        physics_module = import_local_module("so101_public_physics", RL_ROOT / "rsl_rl" / "public_physics.py")
        physics_data = physics_module.load_config(args_cli.physics_config)
        physics_module.apply_config(env_cfg, physics_data)
    ...
    with launch_simulation(env_cfg, args_cli):
        ...
        if agent_cfg.resume or agent_cfg.algorithm.class_name == "Distillation":
            if args_cli.checkpoint and Path(args_cli.checkpoint).is_file():
                resume_path = str(Path(args_cli.checkpoint).resolve())
        ...
        if task_data is not None:
            import json
            (Path(log_dir) / "params" / "public_task.json").write_text(json.dumps(task_data, indent=2) + "\n")
        if physics_data is not None:
            import json
            (Path(log_dir) / "params" / "public_physics.json").write_text(json.dumps(physics_data, indent=2) + "\n")
