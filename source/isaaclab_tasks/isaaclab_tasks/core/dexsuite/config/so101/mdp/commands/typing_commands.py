...
from ..success_monitor.success_monitor_cfg import SuccessMonitorCfg
from . import typing_vis
from .grid_bucket import grid_bucket_downsample
from .keyboard_view import build_keyboard_slot_gather
...
def _resample_reset_kernel(
    ...
    target_slots: wp.array(dtype=wp.int64),
    lo: wp.int32,
    hi: wp.int32,
    seed: wp.int32,
    ...
    m_typeable = int(typeable.shape[0])
    m_target = int(target_slots.shape[0])
    ...
    # Target word: draw from the configured target subset, then pad unused
    # buffer slots with -1. Randomized mistakes below still draw from every
    # typeable key.
    for j in range(width):
        if j < n:
            target[e, j] = target_slots[wp.randi(rng, 0, m_target)]
        else:
    ...
    for j in range(width):
        if j < t:
            if j < n and wp.randf(rng) < match_prob:
                ...
                typed[e, j] = typeable[wp.randi(rng, 0, m_typeable)]
        else:
...
class LetterTypingCommand(CommandTerm):
    ...
    def __init__(self, cfg: LetterTypingCommandCfg, env: ManagerBasedRLEnv):
        ...
        self.num_keys, self._inst_idx, self._col_idx, self._body_col_idx = build_keyboard_slot_gather(
        ...
        target_slots = cfg.typeable_slots if cfg.target_slots is None else cfg.target_slots
        typeable_set = set(int(slot) for slot in cfg.typeable_slots)
        if not target_slots:
            raise ValueError("target_slots cannot be empty")
        if any(int(slot) not in typeable_set for slot in target_slots):
            raise ValueError("target_slots may only contain typeable_slots")
        self._target_slots = torch.tensor(target_slots, dtype=torch.long, device=self.device)
        ...
        if self._cur_enabled:
            ...
            # Beta sampling kernel rate**(a-1) * (1-rate)**(b-1), mode at the target success rate.
            target = min(max(float(cur.beta_target), 0.0), 1.0)
            kappa = max(float(cur.beta_kappa), 0.0)
            self._beta_a = 1.0 + kappa * target
            self._beta_b = 1.0 + kappa * (1.0 - target)
            # Optimistic init: unsampled snapshots sit at the target rate, so they get replayed at least once
            # before their score decays toward their measured (mastered/too-hard) rate.
            self.success_rate = torch.full((cap,), target, dtype=torch.float32, device=self.device)
            monitor_cfg = SuccessMonitorCfg(
                num_monitored_data=cap,
    ...
    def _sample_sources(self, k: int) -> torch.Tensor:
        ...
        still rises toward 1 as snapshots resolve to 0/1 (``mean_score -> 0``). At init all snapshots sit at the
        target, so ``mean_score = peak`` and ``p(normal) = normal_weight / (normal_weight + peak)``.
        """
        rate = self.success_rate.clamp(0.0, 1.0)
        scores = rate.pow(self._beta_a - 1.0) * (1.0 - rate).pow(self._beta_b - 1.0)
        ...
        # rate has collapsed to 0 or 1 (Beta score -> 0); the floor then falls back to ~uniform replay.
    ...
    def _oversample(self, m_candidates: int):
        ...
        wp.launch(
            ...
            inputs=[
                ...
                wp.from_torch(self._target_slots.contiguous(), dtype=wp.int64),
                int(lo),
                int(hi),
    ...
    def _sample_diverse_states(self, cap: int):
        ...
        *absolute* keys-still-to-type (the forward half of the distance-to-success). Together the survivors
        ...
        chosen = valid[grid_bucket_downsample(feat[valid], cap)]
    ...
    def _build_viz_resources(self):
        device = self.device
        # Markers are drawn only for these envs (cfg.viz_env_ids; None -> all). The env cfg sets this to the
        # visualizer's visible_env_indices so markers land on exactly the rendered envs and stay readable.
