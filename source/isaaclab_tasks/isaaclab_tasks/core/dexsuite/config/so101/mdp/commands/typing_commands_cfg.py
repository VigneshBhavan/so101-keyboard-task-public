...
class LetterTypingCommandCfg(CommandTermCfg):
    ...
    class ResetCfg:
        ...
        **oversampling** many candidate typing states, **grid-bucket-downsampling** them to uniform coverage over
        ...
        probability), then running the reset-IK once per survivor. A :class:`~...success_monitor.SuccessMonitor`
        tracks each snapshot's recent success rate, and subsequent resets either take the normal random reset or
        restore a buffered snapshot, biased toward states the policy half-solves.
        """
    ...
    target_slots: tuple[int, ...] | None = None
    """Optional subset of ``typeable_slots`` used to sample target keys.

    ``None`` samples target keys from the complete typeable set. The full
    ``typeable_slots`` set still supplies randomized mistakes, so restricting
    targets does not hide wrong-key or backspace recovery states.
    """
    ...
    every env; set a tuple to restrict. The env cfg sets this to the same indices as the visualizer's
    ``visible_env_indices`` so the markers appear on exactly the rendered envs."""
