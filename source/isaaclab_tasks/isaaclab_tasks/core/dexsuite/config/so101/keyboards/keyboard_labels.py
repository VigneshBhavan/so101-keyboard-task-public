...
def legend_text(label: str, style: KeyboardStyle) -> str:
    ...
    if compact == "SPACE" and not style.label_spacebar:
        return ""
    replacements = (
        _APPLE_LEGEND_REPLACEMENTS if style.name in {"apple_chiclet", "laptop_chiclet"} else _COMMON_LEGEND_REPLACEMENTS
    )
    text = replacements.get(compact, text)
