...
from .keyboard_geometry import base_profile_visuals, inset_box, resolve_keyboard
...
class KeyboardVisuals(Protocol):
    """Custom gprim authoring hooks for mesh-based keyboards (e.g. converted scans).

    Implementations replace the generator's box/tapered-cap gprims while the articulation structure
    (links, joints, drives) stays on the shared authoring path. Newton's shared ArticulationView
    needs uniform per-world shape counts, and the MJWarp solver additionally requires the shape
    *type* sequence to match position-by-position across worlds. Implementations must therefore
    author the generator's exact base sequence - 2 meshes then ``2 + _BASE_VISUAL_SLOTS`` boxes
    (see :func:`_define_base_geometry`) - and exactly (mesh, box, mesh) per active key
    (visual, collision, label)."""

    def author_base(self, stage: Any, base_path: str, resolved: ResolvedKeyboard, cfg: Any, *, collision: bool) -> None:
        """Author the case gprims under ``base_path``."""
        ...

    def author_key(self, stage: Any, link_path: str, key: ResolvedKey, resolved: ResolvedKeyboard, cfg: Any) -> None:
        """Author one key's gprims under ``link_path``."""
        ...
...
def spawn_keyboard(
    ...
    **kwargs,
    ...
    resolved = resolve_keyboard(cfg)
    return author_keyboard(prim_path, resolved, cfg, translation=translation, orientation=orientation, **kwargs)
...
def author_keyboard(
    ...
    *,
    translation: tuple[float, float, float] | None = None,
    orientation: tuple[float, float, float, float] | None = None,
    visuals: KeyboardVisuals | None = None,
    **kwargs,
) -> Any:
    """Author a resolved keyboard into a USD stage, optionally with custom visuals.

    This is the backend shared by :func:`spawn_keyboard` (procedural geometry) and mesh-based
    spawners such as :func:`keyboard_mx_keys.spawn_mx_keys_keyboard`, which pass a resolved
    keyboard plus a :class:`KeyboardVisuals` implementation. The articulation structure - link and
    joint naming, prismatic joints, and drive gains - is identical for every caller.
    """
    ...
    if resolved.partition_mode == "fixed_dof":
        _spawn_partitioned_articulations(stage, prim_path, root, resolved, cfg, visuals)
    else:
        _spawn_single_articulation(stage, prim_path, root, resolved, cfg, visuals)
    return root
...
def _spawn_single_articulation(
    stage, prim_path: str, root, resolved: ResolvedKeyboard, cfg: Any, visuals: KeyboardVisuals | None = None
) -> None:
    ...
    _define_base_geometry(stage, base_path, resolved, cfg, collision=cfg.include_case_collision, visuals=visuals)
...
def _define_base_geometry(
    ...
    *,
    ...
    if visuals is not None:
        visuals.author_base(stage, base_path, resolved, cfg, collision=collision)
        return
    # Two effectively-invisible mesh pads in the slots where mesh-based variants (MX Keys) author
    # their case/trim scan meshes: MJWarp requires the shape-type sequence (mesh vs box) to match
    # position-by-position across worlds, so every pool variant authors mesh, mesh, then boxes.
    _define_pad_mesh(stage, f"{base_path}/case_visual", resolved.style.case_color)
    _define_pad_mesh(stage, f"{base_path}/trim_visual", resolved.style.case_color)
    _define_box(
        stage,
...
def _author_key_articulation(
    ...
    *,
    ...
    if visuals is not None:
        visuals.author_key(stage, link_path, key, resolved, cfg)
    elif key.active:
...
def _define_pad_mesh(stage, path: str, color: tuple[float, float, float]):
    """Author a tiny (effectively invisible) quad mesh used purely as a shape-topology pad."""
    ...
    mesh.CreatePointsAttr(
        Vt.Vec3fArray(
            [Gf.Vec3f(-eps, -eps, 0.0), Gf.Vec3f(eps, -eps, 0.0), Gf.Vec3f(eps, eps, 0.0), Gf.Vec3f(-eps, eps, 0.0)]
        )
    )
