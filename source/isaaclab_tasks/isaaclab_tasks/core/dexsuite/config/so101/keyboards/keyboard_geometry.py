...
def resolve_keyboard(cfg: KeyboardSpawnerCfg) -> ResolvedKeyboard:
    """Resolve a cfg into deterministic in-memory keyboard geometry."""

    _validate_generation_cfg(cfg)
    rng = random.Random(cfg.resolved_seed())
    layout = cfg.layout.class_type(cfg.layout).build(rng)
    family = layout.family
    unit_keys = tuple(layout.keys)
    style = cfg.style.class_type(cfg.style).sample(family, rng)
    deck_quat = _deck_quat(style)
    active_keys = _resolve_active_keys(unit_keys, style, cfg)
...
def _resolve_active_keys(
    unit_keys: Sequence[UnitKey], style: KeyboardStyle, cfg: KeyboardSpawnerCfg
) -> list[ResolvedKey]:
...
def _validate_resolved(keys: Sequence[ResolvedKey], case_size: tuple[float, float, float]) -> list[str]:
    warnings: list[str] = []
