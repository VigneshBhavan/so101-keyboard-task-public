...
if TYPE_CHECKING:
    from .keyboard_gen_cfg import KeyboardLayoutSamplerCfg


LayoutBuilder = Callable[[random.Random], list[UnitKey]]


class KeyboardLayoutSampler:
    """Build logical keyboard layouts from :class:`KeyboardLayoutSamplerCfg`."""

    def __init__(self, cfg: "KeyboardLayoutSamplerCfg"):
        self.cfg = cfg

    def build(self, rng: random.Random) -> LogicalKeyboardLayout:
        """Build a selected logical layout in key-unit coordinates."""

        family = self._sample_family(rng)
        try:
            builder = LAYOUT_BUILDERS[family]
        except KeyError as exc:
            raise ValueError(f"Unsupported keyboard layout family '{family}'.") from exc
        return LogicalKeyboardLayout(family=family, keys=tuple(builder(rng)))

    def _sample_family(self, rng: random.Random) -> str:
        if self.cfg.family != "random":
            return self.cfg.family
        if not self.cfg.families:
            raise ValueError("KeyboardLayoutSamplerCfg.families must contain at least one family.")
        return rng.choice(tuple(self.cfg.families))


def available_layout_families() -> tuple[str, ...]:
    """Return supported layout family names in deterministic order."""

    return tuple(LAYOUT_BUILDERS)
...
def _layout_ansi_60(rng: random.Random) -> list[UnitKey]:
    ...
    rows = [
        _row(
            4,
            [
                ("grave", "`", 1, "alpha"), ("1", "1", 1, "alpha"), ("2", "2", 1, "alpha"),
                ("3", "3", 1, "alpha"), ("4", "4", 1, "alpha"), ("5", "5", 1, "alpha"),
                ("6", "6", 1, "alpha"), ("7", "7", 1, "alpha"), ("8", "8", 1, "alpha"),
                ("9", "9", 1, "alpha"), ("0", "0", 1, "alpha"), ("minus", "-", 1, "alpha"),
                ("equal", "=", 1, "alpha"), ("backspace", "Backspace", 2, "modifier"),
            ],
            row=4,
        ),
        _row(
            3,
            [
                ("tab", "Tab", 1.5, "modifier"), ("q", "Q", 1, "alpha"), ("w", "W", 1, "alpha"),
                ("e", "E", 1, "alpha"), ("r", "R", 1, "alpha"), ("t", "T", 1, "alpha"),
                ("y", "Y", 1, "alpha"), ("u", "U", 1, "alpha"), ("i", "I", 1, "alpha"),
                ("o", "O", 1, "alpha"), ("p", "P", 1, "alpha"), ("left_bracket", "[", 1, "alpha"),
                ("right_bracket", "]", 1, "alpha"), ("backslash", "\\", 1.5, "modifier"),
            ],
            row=3,
        ),
        _row(
            2,
            [
                ("caps_lock", "Caps", 1.75, "modifier"), ("a", "A", 1, "alpha"), ("s", "S", 1, "alpha"),
                ("d", "D", 1, "alpha"), ("f", "F", 1, "alpha"), ("g", "G", 1, "alpha"),
                ("h", "H", 1, "alpha"), ("j", "J", 1, "alpha"), ("k", "K", 1, "alpha"),
                ("l", "L", 1, "alpha"), ("semicolon", ";", 1, "alpha"), ("quote", "'", 1, "alpha"),
        ...
        _row(
            1,
            [
                ("left_shift", "Shift", 2.25, "modifier"), ("z", "Z", 1, "alpha"), ("x", "X", 1, "alpha"),
                ("c", "C", 1, "alpha"), ("v", "V", 1, "alpha"), ("b", "B", 1, "alpha"),
                ("n", "N", 1, "alpha"), ("m", "M", 1, "alpha"), ("comma", ",", 1, "alpha"),
                ("period", ".", 1, "alpha"), ("slash", "/", 1, "alpha"), ("right_shift", "Shift", 2.75, "modifier"),
            ],
            row=1,
        ),
        _row(
            0,
            [
                ("left_ctrl", "Ctrl", 1.25, "modifier"), ("left_meta", "Meta", 1.25, "modifier"),
                ("left_alt", "Alt", 1.25, "modifier"), ("space", "Space", 6.25, "accent"),
                ("right_alt", "Alt", 1.25, "modifier"), ("fn", "Fn", 1.25, "modifier"),
                ("menu", "Menu", 1.25, "modifier"), ("right_ctrl", "Ctrl", 1.25, "modifier"),
            ],
            row=0,
        ),
    ]
...
def _layout_ansi_full(rng: random.Random) -> list[UnitKey]:
    ...
    numpad_x = 20.25
    numpad = [
        _copy_unit_key(key, name=f"num_{key.name}", x=key.x + numpad_x)
        for key in _layout_numpad(random.Random(0))
    ]
    media = _row(
        5,
        [
...
def _ortho_label(row: int, col: int, rows: int, cols: int) -> str:
    if cols == 15:
        labels = (
            (
                "Ctrl", "Meta", "Alt", "Lower", "Fn", "Space", "Space", "Space",
                "Space", "Raise", "Left", "Down", "Up", "Right", "Del",
            ),
...
LAYOUT_BUILDERS: Mapping[str, LayoutBuilder] = {
